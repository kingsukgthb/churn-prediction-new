...
if st.button("Predict", type='primary'):
    response = requests.post(f"{BACKEND_URL}/v1/customer", json=customer_data)    # enter user name and space name before running the cell
...
if file is not None:
    if st.button("Predict for Batch", type='primary'):
        response = requests.post(f"{BACKEND_URL}/v1/customerbatch", files={"file": file})    # enter user name and space name before running the cell
        if response.status_code == 200:
            result = response.json()
